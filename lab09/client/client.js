...
const handleServerMessage = ev => {
    ...
    if (data.type === "login") {
        ...
        console.log(data);
        document.querySelector('#users-list').innerHTML = data.active_users;
        ...
        console.log(data);
        text = data.username === username ? `You: ${data.text}` : `${data.username}: ${data.text}`;
        document.querySelector('#chat').innerHTML += `<div class=${data.username === username ? 'right': 'left'}>${text}</div>`;
    } else {
